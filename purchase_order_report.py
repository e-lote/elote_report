...
class purchase_order_report(osv.osv):
    """ Phone calls by user and section """

    _name = "purchase.order.weblot.report"
    _description = "Purchase orders by partner, product and other attributes"
    _auto = False
    ...
    def init(self, cr):
        ...
        """
        tools.drop_view_if_exists(cr, 'purchase_order_weblot_report')
        cr.execute("""
            create or replace view purchase_order_weblot_report as (
		select b.id,pp.ean13 as ean13,pp.publishing_bs as publishing_bs,pp.paper_colour,pp.default_code as default_code,a.name,
